...
class GUI:
    def __init__(self, master):
        ...
        self.button_remove = Button(self.frame_menu,
                                    text="Remove a spirolateral",
                                    command=self.spiro_remove)
        self.button_draw = Button(
            self.frame_menu, text="Draw a spirolateral",
            command=self.spiro_draw)
        self.button_save = Button(self.frame_menu,
                                  text="Save spirolateral list",
                                  command=self.save)

        self.button_load = Button(self.frame_menu,
                                  text="Load spirolateral list",
                                  command=self.load)
    ...
    def check_add(self):
        ...
        for index in range(len(spiros)):
            if name == spiros[index].name:
                self.label_response1.configure(
                    text="There's already a spirolateral called {}."
                    .format(name))
                name = ""
    ...
    def check_remove(self):
        choice = check_num(self.entry1, self.label_response1,
                           "That integer doesn't correspond to anything.",
                           MIN_CHOICE,
                           len(spiros), int)
    ...
    def spiro_print(self):
        ...
        for index in range(len(spiros)):
            text += "{}) {} - {} segments and {}°\n".format(
                index + 1, spiros[index].name, spiros[index].segment,
                spiros[index].angle)
